...
export const createItem = async (
  ...
  if (!image) {
    ...
    await registerAuditAction(
      'CREAR',
      `Se ha creado un renglón de armamento con el nombre: ${nombre}`
    )
...
export const recoverItem = async (id: number) => {
  ...
  await registerAuditAction(
    'ELIMINAR',
    `Se ha recuperado el renglon de armamento con el id ${id} y el nombre ${exist?.nombre}`
  )
  revalidatePath('/dashboard/armamento/inventario')
...
export const deleteMultipleItems = async (ids: number[]) => {
  ...
  await registerAuditAction(
    'ELIMINAR',
    `Se han eliminado los renglones de armamento con los siguientes ids: ${ids}`
  )
  revalidatePath('/dashboard/armamento/inventario')
