...
import { Componente_Arma } from '@prisma/client'
import { Combobox } from '@/modules/common/components/combobox'
import { useRouter } from 'next/navigation'
import { Loader2 } from 'lucide-react'
import { ComboboxData } from '@/types/types'
import { getAllGunModels } from '../../lib/actions/model-actions'
import { createGunComponent } from '../../lib/actions/component-actions'
interface Props {
  defaultValues?: Componente_Arma
}

type FormValues = Componente_Arma

export default function GunComponentsForm({ defaultValues }: Props) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    ...
    startTransition(() => {
      if (!isEditing) {
        createGunComponent(values).then((data) => {
          if (data?.error) {
            toast({
          ...
          if (data?.success) {
            toast({
              title: 'Componente de arma Creado',
              description: 'El componente de arma se ha creado correctamente',
              variant: 'success',
            })
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="">
                ...
                <FormDescription>
                  Describe el componente de arma de la manera mas detallada
                  posible para tener una mejor búsqueda
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
