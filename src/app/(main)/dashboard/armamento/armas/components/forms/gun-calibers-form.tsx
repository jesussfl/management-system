...
type FormValues = Calibre

export default function GunCalibersForm({ defaultValues }: Props) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    ...
    startTransition(() => {
      if (!isEditing) {
        createGunCaliber(values).then((data) => {
          if (data?.error) {
            toast({
          ...
          if (data?.success) {
            toast({
              title: 'Calibre Creado',
              description: 'El calibre se ha creado correctamente',
              variant: 'success',
            })
      ...
      >
        <div className="px-24">
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="">
                <FormLabel>Nombre del calibre</FormLabel>
                <FormControl>
                  <Input {...field} />
                </FormControl>
                <FormDescription>
