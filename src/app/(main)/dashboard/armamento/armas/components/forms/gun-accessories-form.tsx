...
type FormValues = Accesorio_Arma

export default function GunAccessoriesForm({ defaultValues }: Props) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    ...
    startTransition(() => {
      if (!isEditing) {
        createAccessory(values).then((data) => {
          if (data?.error) {
            toast({
          ...
          if (data?.success) {
            toast({
              ...
              description: 'El accesorio se ha creado correctamente',
              variant: 'success',
            })
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="">
                ...
                <FormDescription>
                  Describe el accesorio de la manera mas detallada posible para
