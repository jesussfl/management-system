...
import { SelectedSerialForLoan } from '@/lib/types/loan-types'

export const CardItemLoan = () => {
        ...
        <FormField
          ...
          render={({ field: { value, onChange, ref, ...field } }) => (
            <FormItem className="flex flex-1 items-center justify-between gap-2">
              <FormLabel className="w-[12rem]">Cantidad a prestar:</FormLabel>
...
const SerialSelectorTrigger = () => {
  ...
  const selectedSerials: SelectedSerialForLoan[] = watch(
    `renglones.${itemIndex}.seriales`
  )

  const isLoanByUnit = !watch(`renglones.${itemIndex}.es_prestamo_liquidos`)
          ...
          <div className="p-24">
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (
                  isSomeFieldEmpty &&
                  watch(`renglones.${itemIndex}.es_prestamo_liquidos`)
                ) {
                  toast({
                    title:
...
const DefaultSerialSelector = ({
  ...
  selectedSerials: SelectedSerialForLoan[]
}) => {
  ...
  const [displaySerials, setDisplaySerials] =
    useState<SelectedSerialForLoan[]>(selectedSerials)
